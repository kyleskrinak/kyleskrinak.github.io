...
- **Quick local iteration** (not for commit): `ALLOW_NATIVE_BASELINE=1 npm run test:visual:baseline` regenerates from macOS rendering — fine for eyeballing a change locally, but don't commit the result or CI will fail on font-height drift. Without the env var the run refuses. The guard is not just the npm script: `playwright.config.ts` screens argv before the build starts, and `tests/global-snapshot-guard.ts` re-checks Playwright's parsed mode as `globalSetup`, so `npm run test:visual -- --update-snapshots`, `npx playwright test -u`, and cluster spellings like `-uall` or `-xu` are all refused on a non-Linux host. Only the overwriting modes (`all`, `changed`) are refused by mode; `missing` is the default for ordinary runs and refusing it would block local testing. It can still create a baseline for a new or renamed test, so a `globalTeardown` deletes any snapshot the run created on a non-Linux host and fails the run -- the emptied `-snapshots` directory goes too, while any directory still holding committed baselines is left alone. Guarded runs are serialized by a lock file: two at once would share this checkout's baselines, so the second refuses rather than racing to delete a snapshot the first is still using. The one way to overwrite committed baselines from macOS is a deliberate `ALLOW_NATIVE_BASELINE=1`.
