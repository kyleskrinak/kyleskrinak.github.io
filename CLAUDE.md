...
3. **Quality gates before push** — `npm run build && npm run check:links && npm run test:visual` (all must pass). **`test:visual` and `test:visual:baseline` must run in Docker, never natively on macOS** — baselines are Docker/CI-rendered; running on macOS produces systematic font-rendering diffs (near-universal 1–2px height mismatches) that look like regressions but aren't. New blog post exceptions: canonical URL 404 (resolves on deploy) and listing-page height diffs are expected (update baselines with `npm run test:visual:baseline` in Docker — ask first); other failures are real. **Docs-only changes** (docs/, README.md): build required; link check and visual optional for trivial edits. Page weight is monitored nightly against production (real uncompressed bytes, 512KB Club methodology, including third-party trackers; transfer bytes also reported) via `weightwatch.yml` — not a push gate; see `scripts/check-live-weight.mjs`.
